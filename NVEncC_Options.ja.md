...
### --y4m-timestamp
y4m出力の各FRAME行に、ストリーム先頭を0秒とする表示時刻を `Xts=<秒>`、表示時間を `Xdur=<秒>` の形式で付加する。表示時間を取得できないフレームでは `Xdur` を省略する。あわせてy4mヘッダに `XTIMEBASE=<num>:<den>` を書き込む。読み込み時にこのタグがあれば、そのtimebaseでVFRとして扱う。
