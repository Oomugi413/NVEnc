...
2026.03.28 (9.13)
- NVENC機能情報の取得結果をGPU単位でキャッシュし、--parallel時の子プロセス側GPU判定やエンコーダ対応判定を安定化。
- マルチチャンネル音声のチャンネルレイアウト判定を改善。
- Dolby Vision関連オプション使用時の注意をドキュメントに追記。( #738 )
...
- --parallel使用時に、Bフレームの利用可能有無が異なるGPUが混在しないように。
- Linuxでmesonを使用したビルドに移行。

2026.01.15 (9.10)
- 音声チャンネル数によって--audio-bitrateで異なる値を指定する機能を追加。
...
[NVEnc.auo]
- 簡易アンインストーラを追加。
  プラグインフォルダの NVEnc_uninstall.bat をダブルクリックで実行すると、下記確認画面で出ます。
...
- プリセットやtuneの設定を表示するオプションを追加。 (--check-preset-params)

2025.10.31 (9.06)
- --vpp-resize bilinear,spline*,lanczos*でリサイズ解像度によっては絵に横方向に線状のノイズが入ることがあったのを修正。
...
- NVEnc.auoからAV1で--refの指定が効かなかった問題を修正。

2025.09.15 (9.03)
- 9.02のパッケージに欠損があったのを修正。
  NVEncNVSDKNGX.dllなどが含まれていなかった。

2025.09.14 (9.02)
- 9.00からngx-vsrとngx-true-hdrを両方使用するとエラー終了してしまっていたのを修正。
...
- --vpp-subburnの焼き込み処理で、libassの初期化方法を変更し、動きのある字幕がガタガタしてしまうのを修正。 ( #717 )
